...
#### 项目介绍
simple-pay源于本人刚好接手同事交接的支付项目，加上本人对支付的理解，利用业余时间全新开发而成。这个项目，大家可以把他理解成一个支付的子系统，并不是一个工具包的概念，contoller层表达的是数据的支付网关数据入口，core层表达的是对各个支付通道各种支付方式对接的具体逻辑，notify表达的是支付回调逻辑，report层是输出统一支付参数通知到其它商户或业务应用子系统等等。 简单付（simple-pay）的目的是让开发者对接第三方支付更加简单，便捷。聚合第三方支付通道并且实现零代码就能聚合收款的目的。<br>
